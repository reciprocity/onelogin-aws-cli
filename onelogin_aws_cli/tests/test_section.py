...
class TestSection(TestCase):
    def test___contains__true(self):
        sec = Section(
            "mock-section", Namespace(has_option=MagicMock(return_value=True))
        )
    ...
    def test___contains__false(self):
        sec = Section(
            "mock-section",
            Namespace(has_option=MagicMock(return_value=False), DEFAULTS=dict()),
        )
    ...
    def test_has_required_false(self):
        sec = Section(
            "mock-section",
            MagicMock(has_option=MagicMock(side_effect=lambda s, x: x != "base_uri")),
        )
    ...
    def test_has_required_true(self):
        sec = Section(
            "mock-section", MagicMock(has_option=MagicMock(return_value=True))
        )
